...
void _INIT ProgramInit(void)
{
	...
	Trajectory_Str.Targets.Position[5] = 100.0;
	
	Trajectory_Str.Targets.Velocity[0] = 300.0;
	Trajectory_Str.Targets.Velocity[1] = 200.0;
	Trajectory_Str.Targets.Velocity[2] = 150.0;
	Trajectory_Str.Targets.Velocity[3] = 200.0;
	Trajectory_Str.Targets.Velocity[4] = 150.0;
	Trajectory_Str.Targets.Velocity[5] = 300.0;
...
void _CYCLIC ProgramCyclic(void)
{
	if(SIMULATION_ENABLE == FALSE){
		...
		switch(state_id){
			...
			case MECH_STATE_WAIT:
				{
					if(Global_VInRoS_Str.Mech_Id_3.Command.Home == TRUE && Global_VInRoS_Str.Mech_Id_3.Info.Move_Active == FALSE){
						state_id = MECH_STATE_HOME_MOTION_1;
					}
				
					if(Global_VInRoS_Str.Mech_Id_3.Command.Start == TRUE && Global_VInRoS_Str.Mech_Id_3.Info.Move_Active == FALSE){
						state_id = MECH_STATE_MOTION_1;
					}
					
					if(Global_VInRoS_Str.Mech_Id_3.Command.Stop == TRUE && Global_VInRoS_Str.Mech_Id_3.Info.Move_Active == TRUE){
						state_id = MECH_STATE_STOP;
					}
		...
		Global_VInRoS_Str.Mech_Id_3.Info.Power = TRUE;
		Global_VInRoS_Str.Mech_Id_3.Info.Home  = TRUE;
	}
}